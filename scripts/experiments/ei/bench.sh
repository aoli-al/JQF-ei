...
screen -S "$SNAME" -p ei_$e -X stuff "JVM_OPTS=$FAST_ENV timeout $TIME $JQF_EI $TIME -c \$($JQF_DIR/scripts/examples_classpath.sh) $TEST_CLASS testWithGenerator $EI_OUT_DIR^M"
...
screen -S "$SNAME" -p ei_no_havoc_$e -X stuff "JVM_OPTS=$NO_HAVOC_ENV timeout $TIME $JQF_EI $TIME -c \$($JQF_DIR/scripts/examples_classpath.sh) $TEST_CLASS testWithGenerator $EI_NO_HAVOC_OUT_DIR^M"
