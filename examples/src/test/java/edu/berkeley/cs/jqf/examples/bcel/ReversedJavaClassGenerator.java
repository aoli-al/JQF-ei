...
public class ReversedJavaClassGenerator extends Generator<JavaClass> {
    ...
    public ReversedJavaClassGenerator() {
    ...
    private Method generateMethod(String className, SourceOfRandomness r) {
        ...
        InstructionList code = generateCode(r, numArgs + 1);
        ...
        int flags = r.nextInt(0, Short.MAX_VALUE);
        MethodGen
                methodGen = new MethodGen(flags, returnType, argTypes, argNames, methodName, className, code, constants);
        // Validate flags
    ...
    private InstructionList generateCode(SourceOfRandomness r, int slots) {
        InstructionList code = new InstructionList();

        ArrayList<Instruction> instructions = new ArrayList<>();

        while (r.nextBoolean()) {
            Instruction ins = generateInstruction(r, slots, code); // TODO: Allocate some locals
    ...
    String generateClassName(SourceOfRandomness r)
    {
        ...
        for (int i = 0; i < numParts; i++) {
            parts[numParts - i - 1] = generateMemberName(r);
        }
    ...
    int generateFieldRef(SourceOfRandomness r) {
        ...
        String memberName = generateMemberName(r);
        String className = generateClassName(r);
